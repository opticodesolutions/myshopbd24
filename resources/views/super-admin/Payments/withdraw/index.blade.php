  ...
  <main class="content">
    <div class="container-fluid p-0">
        <div class="container pt-3 card">
            <div class="row">
                <div class="col-12">
                    <h1 class="text-center mb-4">Withdraw Requests</h1>
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <span class="badge
                                                ...
                                                {{ $withdrawal->status === 'success' ? 'bg-success' : '' }}
                                                {{ $withdrawal->status === 'failed' ? 'bg-danger' : '' }}
                                                {{ $withdrawal->status === 'refund' ? 'bg-info' : '' }}">
                                        ...
                                        <td>
                                            <div class="btn-group" role="group">
                                                @if($withdrawal->status !== 'pending')
                                                    <button class="btn btn-secondary btn-sm" onclick="openModal('pending', {{ $withdrawal->id }}, {{ $withdrawal->user_id }}, '{{ $withdrawal->type }}', '{{ $withdrawal->amount }}', '{{ $withdrawal->payment_method }}')">
                                                        Pending
                                                    </button>
                                                @endif
                                                @if($withdrawal->status !== 'success')
                                                    <button class="btn btn-success btn-sm" onclick="openModal('success', {{ $withdrawal->id }}, {{ $withdrawal->user_id }}, '{{ $withdrawal->type }}', '{{ $withdrawal->amount }}', '{{ $withdrawal->payment_method }}')">
                                                        Success
                                                    </button>
                                                @endif
                                                @if($withdrawal->status !== 'failed')
                                                    <button class="btn btn-danger btn-sm" onclick="openModal('failed', {{ $withdrawal->id }}, {{ $withdrawal->user_id }}, '{{ $withdrawal->type }}', '{{ $withdrawal->amount }}', '{{ $withdrawal->payment_method }}')">
                                                        Failed
                                                    </button>
                                                @endif
                                                @if($withdrawal->status !== 'refund')
                                                    <button class="btn btn-warning btn-sm" onclick="openModal('refund', {{ $withdrawal->id }}, {{ $withdrawal->user_id }}, '{{ $withdrawal->type }}', '{{ $withdrawal->amount }}', '{{ $withdrawal->payment_method }}')">
                                                        Refund
                                                    </button>
                                                @endif
                                            </div>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center">No Withdraw Requests Found</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
